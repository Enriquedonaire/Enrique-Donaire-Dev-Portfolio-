...
export function FloatingPaper({ count = 15 }) {
  ...
  const getPosition = (index: number) => {
    ...
    const x = (col / gridSize + (seed % 1000) / 1000) * dimensions.width
    const y = (row / gridSize + ((seed * 56789) % 1000) / 1000) * dimensions.height
      ...
      {isClient &&
        Array.from({ length: count }).map((_, i) => {
          ...
          return (
            ...
            >
              <div
                className="relative w-20 h-24 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 flex items-center justify-center transform hover:scale-110 transition-transform shadow-xl"
                style={{
